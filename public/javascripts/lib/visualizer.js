    ...
    var sentenceSplit = function(text) {
      ...
      for (var i = 0; i < text.length; i++) {
        ...
        // Have we found the end of a sentence?
          ...
          //---------------------------//
          //-- suisun
          if( requestedData.display[sentenceNo] == "i") {
            display_number[sentenceNo] = rowNo+1;
            display_number2[sentenceNo] = rowNo+1;
            rowNo++;
          }
          else {
            display_number2[sentenceNo] = rowNo-1;
            display_number[sentenceNo] = requestedData.display[sentenceNo];
          } 
          sentenceNo++;
          //---------------------------//
      ...
      sourceData = requestedData.text;
      console.log(requestedData.text);
...
Util.profileStart('chunks');
        ...
        var a1=true,
            a2=true,
            a3=true;
        if(requestedData.display.showNE == 'false')
          a1 = false;
        if(requestedData.display.showWIKI == 'false')
          a2 = false;
        if(requestedData.display.showPOS == 'false')
          a3 = false;
        // suisun 
        $.each(data.spanDrawOrderPermutation, function(spanIdNo, spanId) {
          var span = data.spans[spanId];
          ...
          if( a1 == true && a2 == true && a3 == true ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+2*28;
                return;
              }
              else if( span.id.endsWith('WikiLink') == true ) {
                span.floor = 4+1*28;
                return;
              }
              else {
          ...
          else if( a1 == true && a2 == false && a3 == true ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+1*28;
                return;
              }
          ...
          else if( a1 == true && a2 == false && a3 == false ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+0*28;
                return;
              }
          ...
          else if( a1 == false && a2 == true && a3 == true ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+0*28;
                return;
              }
          ...
          else if( a1 == false && a2 == true && a3 == false ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+0*28;
                return;
              }
          ...
          else if( a1 == false && a2 == false && a3 == true ){
              if( span.id.endsWith('NE') == true ) {
                span.floor = 4+0*28;
                return;
              }
...
Util.profileStart('rows');
        ...
        $.each(rows, function(rowId, row) {
          ...
          if (data.markedSent[currentSent]) {
            ...
            //-- suisun
            let bgClassNum;
            let num = Object.keys(display_number2).length;
            if (display_number2[currentSent] == undefined) {
              bgClassNum = display_number2[num - 1];
            } else {
              bgClassNum = display_number2[currentSent];
              //-- watch the last circle of iteration...
              if(num > rows.length && currentSent == rows.length) {
                bgClassNum = display_number2[num - 1];
              }
            }
            bgClass = 'background' + bgClassNum % 2;
            //----------------------------//
          } else {
            // plain "standard" bg
            bgClass = 'background0';
          ...
          if (row.sentence) {
            ...
            var text = svg.text(link, sentNumMargin - Configuration.visual.margin.x, y - rowPadding,
                '' + display_number[row.sentence-1], { 'data-sent': row.sentence});
            //----------------------------//
...
Util.profileStart('before render');
      ...
      if (!Visualizer.areFontsLoaded) {
        var webFontConfig = {
          custom: {
            ...
            urls: webFontURLs !== undefined ? webFontURLs : [
              //----------------------------//
              //-- changed with local ref | suisun
              './stylesheets/static/fonts/Astloch-Bold.ttf',
              './stylesheets/static/fonts/PT_Sans-Caption-Web-Regular.ttf',
              './stylesheets/static/fonts/Liberation_Sans-Regular.ttf'
              //----------------------------//
            ],
          },
        ...
        setTimeout(function() {
          if (!Visualizer.areFontsLoaded) {
            //----------------------------//
            //-- temporarily commented to avoid warning | suisun
