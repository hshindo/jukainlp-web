...
export default class LineComponent extends React.Component {
    ...
    render() {
        let renderForm = this.props.word.words.map((item, index) => {
            ...
            return (
                <div key={index} style={{width: this.props.word.words.length == 1 ? '100%':'auto'}} className="item-cat">
                    <div onMouseOver={mouseOverCat}
                         onMouseOut={mouseOutCat}
                         style={{backgroundColor: color}} className="cat cursor-pointer">
                        {item.cat}
                    </div>
            ...
            <div className="padding-word-wrapper">
                <div className="padding-word">
                    {(this.props.word.ne
                            ...
                            style={{background: this.props.word.bgColor}} className="word-ne cursor-pointer">{this.props.word.ne}</div>
                            :''
                    )}
